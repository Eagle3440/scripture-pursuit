...
This report helps reinforce biblical knowledge, track your progress over multiple play sessions, and see how close you came to beating your personal high score.
...
* **What it does:** Clears your used question history and reshuffles all 400+ questions back into the active deck.
* **When to use it:** If you're seeing too many repeat questions within a session, or if you want to start fresh with a completely randomized question order.
* **Storage:** Clears the `bulletline_used_questions` localStorage entry and resets the in-memory tracking.
* **Note:** This button only appears on the title screen (not during active gameplay). Start a game first if you want to reshuffle between rounds.
